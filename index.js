...
const uri =
  'mongodb+srv://strombergmarkus:testPass@grubwheel.ukqemrm.mongodb.net/?retryWrites=true&w=majority';
...
async function run() {
  try {
    ...
    await createIngredient(client, {
      name: 'Paprika, grön',
      category: 'fruitAndVeg',
      image: './images/paprika-green.jpg',
      addedToListDates: ['test', 'test2'],
    });
...
async function createIngredient(client, newIngredient) {
  ...
  console.log(
    `New ingredient created with the following id: ${result.insertedId}`
  );
}
